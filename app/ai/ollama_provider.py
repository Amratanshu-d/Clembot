...
class OllamaProvider(AIProvider):
    ...
    Enforces a single-inference lock and conservative context/token limits.
    """
    ...
    def plan(self, command: str, context: ScreenContext) -> AgentPlan:
        with _ollama_lock:
            try:
                ...
                res = httpx.post(f"{settings.ollama_host}/api/chat", json=payload, timeout=20.0)
                if res.status_code == 200:
                    ...
                    try:
                        ...
                        retry_res = httpx.post(f"{settings.ollama_host}/api/chat", json=retry_payload, timeout=20.0)
